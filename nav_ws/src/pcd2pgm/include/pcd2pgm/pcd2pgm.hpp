...
private:
  ...
  float estimateGroundZ(std::vector<float> & values) const;

  static int clampOccupancy(int value);

  int computeTerrainOccupancy(float obstacle_height, float obstacle_ratio, float height_diff, float slope) const;
  ...
  std::string ground_estimation_method_;
  float ground_cluster_tolerance_;
  float ground_cluster_percentile_;
  float ground_layer_max_gap_;
  float ground_layer_count_ratio_;
  float obstacle_percentile_;
  float flat_step_threshold_;
  int step_cost_max_;
  int min_points_per_cell_;
  float obstacle_min_height_;
  float obstacle_ratio_threshold_;
  float max_slope_traversable_;
  float slope_cost_start_;
